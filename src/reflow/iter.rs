...
impl<'s> Warp<'s> {
    ...
    fn craft_line(&mut self) -> (usize, String, Range) {
        ...
        let line_content = self.queue.drain(..).map(|(range, s)| {
...
impl<'s> Iterator for Warp<'s> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let offset = 0usize; // @todo derive the indent based on the line spans
        'outer: while let Some((char_idx,(byte_offset, c))) = self.inner.next() {
            'unbreakable: while let Some(unbreakable) = self.unbreakable_ranges.get(self.unbreakable_idx) {
            ...
            let byte_range = if let Some(&(char_idx, (byte_offset_next, c_next))) = self.inner.peek() {
            ...
            let acc_len = if let Some((first, _)) = qiter.next() {
                qiter.map(|(r,_)| r.len()).fold(first.len(), |acc, len| { acc + 1 + len})
            } else {
                0usize
            };
            ...
            // into lines
            let item = (char_range.clone(), std::borrow::Cow::Borrowed(dbg!(&self.s[byte_range])));

            let ret = if offset + acc_len <= self.max_line_width {
                log::warn!("Line is longer than anticipated {} vs. {}", acc_len, self.max_line_width);

                if offset + acc_len + char_range.len() >= self.max_line_width {
            ...
            } else if char_range.len() > self.max_line_width {
                log::warn!("A unbreakable chunk is larger than the max line width {} vs {}", char_range.len(), self.max_line_width);
                if acc_len > 0 {
...
mod tests {
    ...
    fn verify_reflow(
        ...
        let mut warp = Warp::new(content, 0..content.len(), max_line_width);

        warp.add_unbreakables(unbreakables);

        for ((line_no, line_content, _), (expected_no, expected_content)) in warp.zip(expected) {
